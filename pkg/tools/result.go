...
// SuccessResult creates a successful ToolResult with content for the LLM.
// This is a convenience alias for NewToolResult.
//
// Example:
//
//	result := SuccessResult("Operation completed successfully")
func SuccessResult(message string) *ToolResult {
	return &ToolResult{
		ForLLM:  message,
		Silent:  false,
		IsError: false,
...
// WithForUser sets the ForUser field and returns the result for chaining.
// This allows fluent API usage when building results.
//
// Example:
//
//	result := SuccessResult("File saved").WithForUser("✅ File saved successfully")
func (tr *ToolResult) WithForUser(content string) *ToolResult {
	tr.ForUser = content
	return tr
}
