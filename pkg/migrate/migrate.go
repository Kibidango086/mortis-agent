...
func Run(opts Options) (*Result, error) {
	...
	MortisAgentHome, err := resolveMortisAgentHome(opts.MortisAgentHome)
	if err != nil {
		return nil, err
	}
	...
	actions, warnings, err := Plan(opts, openclawHome, MortisAgentHome)
	if err != nil {
		return nil, err
	}

	fmt.Println("Migrating from OpenClaw to mortisagent")
	fmt.Printf("  Source:      %s\n", openclawHome)
	...
	result := Execute(actions, openclawHome, MortisAgentHome)
	result.Warnings = warnings
	return result, nil
}

func Plan(opts Options, openclawHome, MortisAgentHome string) ([]Action, []string, error) {
	var actions []Action
	var warnings []string
	...
	if !opts.WorkspaceOnly {
		...
		if err != nil {
			...
			actions = append(actions, Action{
				...
				Source:      configPath,
				Destination: filepath.Join(MortisAgentHome, "config.json"),
				Description: "convert OpenClaw config to mortisagent format",
			})
	...
	if !opts.ConfigOnly {
		...
		dstWorkspace := resolveWorkspace(MortisAgentHome)
...
func Execute(actions []Action, openclawHome, MortisAgentHome string) *Result {
	result := &Result{}

	for _, action := range actions {
		switch action.Type {
		case ActionConvertConfig:
			if err := executeConfigMigration(action.Source, action.Destination, MortisAgentHome); err != nil {
...
func executeConfigMigration(srcConfigPath, dstConfigPath, MortisAgentHome string) error {
	...
	if _, err := os.Stat(dstConfigPath); err == nil {
		...
		if err != nil {
			return fmt.Errorf("loading existing mortisagent config: %w", err)
		}
...
func resolveMortisAgentHome(override string) (string, error) {
	if override != "" {
		return expandHome(override), nil
	}
	if envHome := os.Getenv("MORTIS_AGENT_HOME"); envHome != "" {
		return expandHome(envHome), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
	...
	return filepath.Join(home, ".mortis-agent"), nil
}
